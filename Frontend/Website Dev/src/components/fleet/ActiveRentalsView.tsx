...
export function ActiveRentalsContent({ sessions, telematics, isDarkMode }: { sessions: any[], telematics: any, isDarkMode: boolean }) {
...
export function ActiveRentalsView({ sessions, telematics, isDarkMode }: { sessions: any[], telematics: any, isDarkMode: boolean }) {
  ...
  return (
    <div className="flex-1 flex gap-6 overflow-hidden mt-6 relative">
      <main className={`flex-col min-w-0 overflow-y-auto no-scrollbar gap-4 pb-4 transition-all duration-500 ease-in-out ${isExpanded ? 'hidden w-0 opacity-0 translate-x-[-100%]' : 'flex-1 flex opacity-100 translate-x-0'}`}>
        <ActiveRentalsContent sessions={sessions} telematics={telematics} isDarkMode={isDarkMode} />
      </main>
